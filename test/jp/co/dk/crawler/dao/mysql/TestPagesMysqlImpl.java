...
public class TestPagesMysqlImpl extends TestCrawlerFoundation{
	...
	public void select() throws DataStoreManagerException {
		...
		try {
			// 登録処理を実行
			PagesRecord record = pages.select(protcol, host, path, filename, parameter, fileid, timeid);
		...
		try {
			// 登録処理を実行
			List<PagesRecord> records = pages.select(protcol, host, path, filename, parameter);
			...
			assertEquals(records.get(0).getH_filename(), filename.hashCode());
	...
	public void count() throws DataStoreManagerException {
		...
		try {
			// 登録処理を実行
			int count = pages.count(protcol, host, path, filename, parameter);
		...
		try {
			// 登録処理を実行
			int count = pages.count(protcol, host, null, null, null);
			assertEquals(count, 1);
		} catch (DataStoreManagerException e) {
			fail(e);
		}
