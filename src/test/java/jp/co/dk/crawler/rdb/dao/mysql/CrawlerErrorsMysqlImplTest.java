...
import jp.co.dk.crawler.rdb.dao.record.UrlsRecord;
...
public class CrawlerErrorsMysqlImplTest extends CrawlerFoundationTest{
	...
	public void count() throws DataStoreManagerException {
		...
		try {
			...
			// 登録されていない場合、０件が返されること。
			assertEquals(crawlerErrors.count(protcol, host, path, parameter), 0);
			...
			// 引数に正常値を渡した場合、正常に取得できること。
			try {
				assertEquals(crawlerErrors.count(protcol, host, path,  parameter),1);
			} catch (DataStoreManagerException e) {
				fail(e);
			}
			...
			// 引数に空のパスリスト、パラメータマップを渡した場合、正常に取得できること。
			try {
				assertEquals(crawlerErrors.count(protcol, host, null,  null),1);
			} catch (DataStoreManagerException e) {
				fail(e);
			}
		} finally {
